...
// Yeebo-specific initialization sequence for 800x1280 display
const ili9881c_lcd_init_cmd_t vendor_specific_init_yeebo[] = {
    // {cmd, { data }, data_size, delay_ms}
    /**** CMD_Page 3 - Yeebo specific timing ****/
    ...
    {0x03, (uint8_t []){0x73}, 1, 0},    // Modified for Yeebo
    {0x04, (uint8_t []){0x73}, 1, 0},    // Modified for Yeebo
    {0x05, (uint8_t []){0x03}, 1, 0},    // Modified for Yeebo
    {0x06, (uint8_t []){0x04}, 1, 0},
    {0x07, (uint8_t []){0x02}, 1, 0},
    {0x08, (uint8_t []){0x02}, 1, 0},
    {0x09, (uint8_t []){0x00}, 1, 0},
    {0x0a, (uint8_t []){0x01}, 1, 0},    // Modified for Yeebo
    {0x0b, (uint8_t []){0x01}, 1, 0},    // Modified for Yeebo
    {0x0c, (uint8_t []){0x00}, 1, 0},
    {0x0d, (uint8_t []){0x01}, 1, 0},    // Modified for Yeebo
    {0x0e, (uint8_t []){0x01}, 1, 0},    // Modified for Yeebo
    {0x0f, (uint8_t []){0x00}, 1, 0},
    // Continue with Yeebo-optimized values...
    {ILI9881C_CMD_CNDBKxSEL, (uint8_t []){ILI9881C_CMD_BKxSEL_BYTE0, ILI9881C_CMD_BKxSEL_BYTE1, ILI9881C_CMD_BKxSEL_BYTE2_PAGE1}, 3, 0},
    {0x22, (uint8_t []){0x0A}, 1, 0},    // Modified MIPI lane timing
    {0x31, (uint8_t []){0x00}, 1, 0},
    {0x40, (uint8_t []){0x33}, 1, 0},    // Modified for better signal integrity
    {0x50, (uint8_t []){0x96}, 1, 0},    // Modified VCOM setting
    {0x51, (uint8_t []){0x96}, 1, 0},    // Modified VCOM setting
    {0x53, (uint8_t []){0x4C}, 1, 0},    // Modified for Yeebo
    {0x55, (uint8_t []){0x4F}, 1, 0},    // Modified for Yeebo
    {0x60, (uint8_t []){0x07}, 1, 0},    // Modified timing
    {0x2E, (uint8_t []){0xF0}, 1, 0},    // Modified for Yeebo
    // Back to CMD_Page 0
    ...
    {0x29, (uint8_t []){0x00}, 0, 20},   // Display on with delay
};

const size_t vendor_specific_init_yeebo_size = sizeof(vendor_specific_init_yeebo) / sizeof(ili9881c_lcd_init_cmd_t);
